...
describe("stage 5.8.5.4 gate (workflow step 04 - supervision portfolio)", () => {
  it("describes supervision portfolio overview", () => {
    const source = readPublicHomePage();

    expect(source).toContain('title: "תיק פיקוח — תמונת מצב"');
    expect(source).toContain("ליקויים שפורסמו");
