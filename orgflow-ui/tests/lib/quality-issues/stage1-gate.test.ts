...
describe("stage 1 gate UI wiring", () => {
  it("connects issues table photos and portfolio QC summary panel", () => {
    ...
    expect(portfolioPanel).toContain("דוח אחרון");
    ...
    expect(portfolioPage).toContain("PORTFOLIO_SUPERVISION_PAGE_EYEBROW");
    expect(portfolioHelpers).toContain("rankProjectsBySupervisionPressure");
  });
});
